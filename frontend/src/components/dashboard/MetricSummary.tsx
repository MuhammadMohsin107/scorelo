      ...
      {standardMetrics.map((metric) => {
        ...
        return (
          <div key={metric.id} className={layout === 'column'
            ...
            : 'flex items-center gap-3 py-2 sm:px-3.5 sm:py-1 first:sm:pl-0 last:sm:pr-0'}>
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-[11.5px] font-medium text-surface-500">{metric.label}</p>
              <div className="mt-0.5 flex items-baseline gap-2">
                <p className="text-[16px] font-semibold tracking-tight text-surface-950 tabular-nums">
                  {metric.value.toLocaleString()}
                  {metric.suffix && <span className="ml-1 text-[11.5px] font-medium text-surface-400">{metric.suffix}</span>}
                </p>
