...
export default function DashboardEmpty({ onAuditComplete }: Props) {
  ...
  return (
    <div className="page-shell">
      <div className="card flex min-h-[280px] flex-col items-center justify-center border border-surface-200 p-6 text-center">
        <div className="mb-2.5 flex h-12 w-12 items-center justify-center rounded-lg bg-brand-50">
          {state === 'running' ? (
        ...
        <h2 className="mb-2 text-[15px] font-semibold text-surface-900">
          {state === 'running' ? 'Analyzing your store' : 'No audits yet'}
        </h2>
        <p className="mb-2 max-w-md text-[12.5px] text-surface-500">
          {state === 'running'
        ...
        {state === 'running' && (
          <div className="mb-2 w-full max-w-xs">
            ...
            <p className="mt-2 text-[11.5px] text-surface-400">{progress}% complete</p>
          </div>
        )}

        {error && (
          <div className="mb-2 flex max-w-md items-start gap-2.5 rounded-lg border border-critical-200 bg-critical-50 p-3.5 text-left">
            <AlertCircle size={16} className="mt-0.5 shrink-0 text-critical-500" />
            <div>
              <p className="text-[12.5px] text-critical-700">{error.message}</p>
              {error.needsConnection && (
                <Link
                  to="/integrations"
                  className="mt-2 inline-flex items-center gap-1.5 text-[12.5px] font-medium text-critical-700 underline underline-offset-2"
                >
                  <PlugZap size={14} />
                  Connect your store
                </Link>
        ...
        <p className="mt-2 inline-flex items-center gap-1.5 text-[11.5px] text-surface-400">
          <Globe size={12} />
