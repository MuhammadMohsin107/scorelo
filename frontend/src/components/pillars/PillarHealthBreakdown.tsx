...
/** Shared breakdown card used in the right column of every pillar sub-page. */
export default function PillarHealthBreakdown({
  title,
  total,
  items = [],
  rows,
  ...
  if (status === 'loading') {
    return (
      <div className="rounded-xl border border-surface-200 bg-white p-6 shadow-[0_8px_24px_-20px_rgba(15,23,42,0.45)] animate-pulse">
  ...
  if (status === 'error') {
    return (
      <div className="rounded-xl border border-surface-200 bg-white p-6 shadow-[0_8px_24px_-20px_rgba(15,23,42,0.45)]">
  ...
  return (
    <div className="rounded-xl border border-surface-200 bg-white p-5 shadow-[0_8px_24px_-20px_rgba(15,23,42,0.45)] sm:p-6">
      <div className="mb-6 flex items-start justify-between gap-3">
        <div>
          <div className="mb-1 text-[10px] font-bold uppercase tracking-[0.14em] text-brand-600">Coverage overview</div>
          <h2 className="text-lg font-bold tracking-tight text-surface-900">{title}</h2>
        </div>
        <span className="rounded-full bg-success-50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-success-700">Live</span>
      </div>
              ...
              <div key={row.label} className="space-y-2">
                <div className="flex items-center justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    {row.count > 0 && (
                      <div className="text-[11px] text-surface-500">{row.count.toLocaleString()} items</div>
                    )}
                  </div>
      ...
      {!showStructuredRows && items.length > 0 && (
        <div className="space-y-3">
          {items.map((item) => {
            ...
            return (
              <div key={item.label} className="flex items-center justify-between rounded-lg border border-surface-100 bg-surface-50 p-4">
