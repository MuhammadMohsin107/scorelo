...
export function MetricTile({ label, value, detail, tone = 'neutral' }: { label: string; value: string | number; detail?: string; tone?: 'neutral' | 'success' | 'warning' | 'critical' | 'info' }) {
  ...
  return (
    <div className={`rounded-lg border bg-surface-0 p-3 shadow-[0_8px_24px_-20px_rgba(15,23,42,0.45)] ${toneClass}`}>
      <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-surface-500">{label}</p>
      <p className="mt-2 text-[20px] font-bold tracking-tight text-surface-950 tabular-nums">{value}</p>
      {detail && <p className="mt-1 text-[11.5px] text-surface-500">{detail}</p>}
    </div>
  );
}
...
export function ModuleHeader({ eyebrow, title, description, actions }: { eyebrow: string; title: string; description: string; actions?: ReactNode }) {
  return (
    <div className="page-head border-b border-surface-200 pb-2.5">
  ...
  return (
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-labelledby="workflow-drawer-title">
      ...
      <aside className="absolute right-0 top-0 flex h-full w-full max-w-xl flex-col border-l border-surface-200 bg-surface-0 shadow-2xl">
        <div className="flex items-start justify-between border-b border-surface-200 px-3.5 py-2.5">
          <div>
            {eyebrow && <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-brand-600">{eyebrow}</p>}
            <h2 id="workflow-drawer-title" className="mt-1 text-[16px] font-bold tracking-tight text-surface-950">{title}</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-3.5 py-2">{children}</div>
      </aside>
    </div>
  );
}
