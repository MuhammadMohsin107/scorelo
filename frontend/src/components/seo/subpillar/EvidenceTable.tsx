      ...
      <div className={`${cardHeader} px-6 py-4`}>
        ...
        <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center">
          <label className="relative block lg:max-w-xs lg:flex-1">
            ...
            <input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={evidence.searchPlaceholder} className="h-9 w-full rounded-lg border border-surface-200 bg-surface-50 pl-9 pr-3 text-sm text-surface-900 outline-none transition-colors placeholder:text-surface-400 focus:border-brand-400 focus:bg-surface-0 focus:ring-2 focus:ring-brand-100" />
          </label>
          <div className="flex flex-wrap items-center gap-2">
            {evidence.facet && <label className="inline-flex items-center gap-1.5 text-xs text-surface-500"><SlidersHorizontal size={13} aria-hidden="true" /><span className="sr-only sm:not-sr-only">{evidence.facet.label}</span><select value={facet} onChange={(event) => setFacet(event.target.value)} className="h-9 cursor-pointer rounded-lg border border-surface-200 bg-surface-0 px-2 text-xs font-medium text-surface-700 outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100"><option value="All">{evidence.facet.allLabel}</option>{evidence.facet.values.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>}
            {evidence.sorts.length > 0 && <label className="inline-flex items-center gap-1.5 text-xs text-surface-500"><span className="sr-only sm:not-sr-only">Sort</span><select value={sortKey} onChange={(event) => setSortKey(event.target.value)} className="h-9 cursor-pointer rounded-lg border border-surface-200 bg-surface-0 px-2 text-xs font-medium text-surface-700 outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100">{evidence.sorts.map((option) => <option key={option.key} value={option.key}>{option.label}</option>)}</select></label>}
          </div>
        </div>
