      ...
      <div className={`${cardHeader} px-6 py-4`}>
        ...
        <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {evidence.sorts.length > 0 && <label className="inline-flex items-center gap-1.5 text-xs text-surface-500"><span className="sr-only sm:not-sr-only">Sort</span><select value={sortKey} onChange={(event) => setSortKey(event.target.value)} className="h-9 cursor-pointer rounded-lg border border-surface-200 bg-white px-2 text-xs font-medium text-surface-700 outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100">{evidence.sorts.map((option) => <option key={option.key} value={option.key}>{option.label}</option>)}</select></label>}
          </div>
        </div>
