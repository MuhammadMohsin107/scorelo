...
export default function FixCenter() {
  ...
  return (
    <div className="page-shell section-stack">
      ...
      <section className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5" aria-label="Finding summary">
      ...
      <section className="space-y-2" aria-labelledby="priority-findings-title">
        <SectionHeading eyebrow="Priority queue" title="Findings to work through" description="Review evidence first, then choose the smallest supported action." action={selectedIds.length > 0 ? <Button onClick={markSelectedReviewed}><Check size={14} />Mark reviewed ({selectedIds.length})</Button> : undefined} />
        {/* One 26px row of filters, matching the control height used everywhere else, instead of
            three 38px controls in a 12px-padded card above the queue. */}
        <div className="flex flex-col gap-2 rounded-lg border border-surface-200 bg-surface-0 px-2.5 py-2 shadow-[0_8px_24px_-20px_rgba(15,23,42,0.45)] lg:flex-row lg:items-center">
          <label className="relative min-w-0 flex-1"><Search size={14} className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-surface-400" /><span className="sr-only">Search findings</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search issue, pillar, or sub-pillar" className="w-full rounded-md border border-surface-200 bg-surface-50 py-1 pl-8 pr-2.5 text-[11.5px] outline-none transition focus:border-brand-400 focus:bg-surface-0 focus:ring-2 focus:ring-brand-100" /></label>
          <div className="flex flex-wrap items-center gap-1.5"><SlidersHorizontal size={14} className="flex-shrink-0 text-surface-400" aria-hidden="true" />
            <select value={pillar} onChange={(event) => setPillar(event.target.value)} aria-label="Filter by pillar" className="rounded-md border border-surface-200 bg-surface-0 px-2 py-1 text-[11.5px] text-surface-700 outline-none focus:border-brand-400"><option>All pillars</option>{['SEO', 'Content', 'Speed', 'CRO', 'AI Discovery'].map((item) => <option key={item}>{item}</option>)}</select>
            <select value={severity} onChange={(event) => setSeverity(event.target.value)} aria-label="Filter by severity" className="rounded-md border border-surface-200 bg-surface-0 px-2 py-1 text-[11.5px] text-surface-700 outline-none focus:border-brand-400"><option>All severity</option>{['critical', 'high', 'medium', 'low'].map((item) => <option key={item}>{item}</option>)}</select>
            <select value={status} onChange={(event) => setStatus(event.target.value)} aria-label="Filter by status" className="rounded-md border border-surface-200 bg-surface-0 px-2 py-1 text-[11.5px] text-surface-700 outline-none focus:border-brand-400"><option>All status</option>{['open', 'reviewed', 'resolved', 'ignored'].map((item) => <option key={item}>{item}</option>)}</select>
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded-lg border border-surface-200 bg-surface-0 shadow-[0_8px_24px_-20px_rgba(15,23,42,0.45)]">
          <div className="hidden grid-cols-[32px_minmax(220px,1.8fr)_0.8fr_0.9fr_0.7fr_0.7fr_0.8fr_86px] gap-2.5 border-b border-surface-200 bg-surface-50 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-surface-500 lg:grid"><span><input type="checkbox" checked={allSelected} onChange={() => setSelectedIds(allSelected ? [] : filteredFindings.map((finding) => finding.id))} aria-label="Select all findings" /></span><span>Issue</span><span>Pillar</span><span>Sub-pillar</span><span>Severity</span><span>Affected</span><span>Status</span><span /></div>
          {filteredFindings.length === 0 ? (
            <div className="px-3 py-7 text-center">
              <Sparkles size={18} className="mx-auto text-success-600" />
              <h3 className="mt-1.5 text-[12.5px] font-bold text-surface-900">No issues found</h3>
              <p className="mt-0.5 text-[11.5px] text-surface-500">Try a different filter or search term.</p>
            </div>
          ) : (
            /* ── One row, two shapes ────────────────────────────────────
            ...
            <div className="divide-y divide-surface-100">
              {filteredFindings.map((finding) => (
                <div
                  key={finding.id}
                  className="flex gap-2.5 px-3 py-1.5 transition hover:bg-surface-50 lg:grid lg:grid-cols-[32px_minmax(220px,1.8fr)_0.8fr_0.9fr_0.7fr_0.7fr_0.8fr_86px] lg:items-center lg:gap-2.5"
                >
      ...
      <section className="rounded-lg border border-surface-200 bg-surface-0 p-2.5 shadow-[0_8px_24px_-20px_rgba(15,23,42,0.45)]">
        <SectionHeading eyebrow="History" title="Applied fixes" description="A lightweight record of actions taken in this audit workspace." />
        <div className="mt-2 divide-y divide-surface-100">
          {findings.filter((finding) => finding.status !== 'open').map((finding) => (
            <div key={finding.id} className="flex flex-wrap items-center justify-between gap-2 py-1.5">
              <div className="min-w-0">
                <p className="text-[12px] font-bold text-surface-900">{finding.title}</p>
                <p className="text-[11px] text-surface-500">
                  {statusLabel[finding.status]}
                  {finding.statusChangedAt && <><span className="mx-1 text-surface-300">|</span>{new Date(finding.statusChangedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</>}
                </p>
              </div>
              <span className="text-[12px] font-bold text-success-700">Potential +{finding.scoreLift} pts</span>
            </div>
          ))}
        </div>
      </section>
    ...
    findings one by one, so it is the last place the capability should be missing. */}<AiRecommendationPanel findingId={selectedFinding.id} /></div><div className="grid grid-cols-2 gap-2"><MetricTile label="Affected" value={selectedFinding.affected.toLocaleString()} detail={selectedFinding.affectedLabel} tone="critical" /><MetricTile label="Potential lift" value={`+${selectedFinding.scoreLift}`} detail="Score points" tone="success" /></div><div className="flex flex-wrap gap-2"><Button onClick={() => updateFinding(selectedFinding.id, 'reviewed')}><Check size={15} />Mark reviewed</Button><Button variant="secondary" onClick={() => updateFinding(selectedFinding.id, 'ignored')}><X size={15} />Ignore</Button><Button variant="ghost" onClick={() => setSelectedFinding(null)}>Close</Button></div></div>}
      </Drawer>
    </div>
  );
}
