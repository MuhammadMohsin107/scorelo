...
import { fetchIntegrations, updateIntegrationStatus, type IntegrationRecord } from '../data/integrations.repository';
...
export default function Integrations() {
  ...
  const sortedSyncTimes = records
    .map((record) => record.lastSynced)
    .filter((value) => value !== 'Not available')
    .sort();
  const mostRecentSync = sortedSyncTimes[sortedSyncTimes.length - 1];

  const sync = (record: IntegrationRecord) => {
    setSyncingId(record.id);
    updateIntegrationStatus(record.id, 'Connected')
      .then((updated) => {
  ...
  const connect = (record: IntegrationRecord) => {
    updateIntegrationStatus(record.id, 'Connected', { notice: null })
      .then((updated) => {
        setRecords((current) => current.map((item) => item.id === updated.id ? updated : item));
        setSelected(updated);
      })
  ...
  const disconnect = (record: IntegrationRecord) => {
    updateIntegrationStatus(record.id, 'Not Connected', { notice: null })
      .then((updated) => {
        setRecords((current) => current.map((item) => item.id === updated.id ? updated : item));
        setSelected(null);
      })
      .catch((error) => console.error('Failed to disconnect integration', error));
  };
