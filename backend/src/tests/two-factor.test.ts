...
/**
 * ─── Two-factor authentication · what can be proven without a database ───
 *
 * Covers BOTH SIDES of one system, in one file because they are one system: the customer flow
 * (sign in, resend, toggle) and the admin flow that manages and monitors it. Splitting them would
 * invite a change to one side that quietly contradicts the other.
 *
 ...
 * would only prove the stub behaves. The same goes for the admin aggregates and the two privileged
 * writes.
 *
 * What IS provable is the boundary, and it is where this feature could actually go wrong:
 *
 *   · the INPUT CONTRACTS that decide what a request may carry, customer and admin alike
 *   · the EMAIL that leaves the building
 *   · the AUTHORIZATION DECISION, which is a pure function of the row that was read
 *   · the LIFECYCLE MAPPER that decides what an operator is told about a challenge
 *   · the REDACTION and VOCABULARY invariants, checked against the source itself because their
 *     failure modes are silent — a hash added to a select leaks, and an event type missing from
 *     the CHECK constraint makes the audit write fail inside a never-throwing logger
 */
...
// ═══ Admin side ══════════════════════════════════════════════════════
// The operator surface over the SAME system above: /api/admin/two-factor. It issues no code,
// mints no ticket and defines no second factor of its own — so what is worth asserting here is
// who may reach it, what a privileged request may carry, and what an operator is allowed to see.
...
describe('admin privileged action contract', () => {
...
describe('admin target id contract', () => {
...
describe('admin monitoring query contracts', () => {
...
describe('admin challenge lifecycle reporting', () => {
  const base = {
    id: 12,
    userId: 3,
    purpose: 'login_2fa',
  ...
  it('calls a live challenge open', () => {
    assert.equal(toAdminChallengeRow(base, new Date('2026-09-03T09:05:00.000Z')).status, 'open');
  });

  it('calls a lapsed challenge expired', () => {
    assert.equal(toAdminChallengeRow(base, new Date('2026-09-03T09:10:00.001Z')).status, 'expired');
...
// Both of these fail SILENTLY in production, which is why they are asserted rather than left to
// review: a hash added to a select would simply appear in a response, and an event type missing
 ...
 *
 * The redaction check scans for credential column names, and these files NAME those columns in
 * prose precisely to record that they are not selected — "`code_hash` is not selected, not
 * redacted downstream" is documentation of the rule, not a violation of it. Scanning raw text
 * would fail on the very comment that exists to prevent the bug.
 */
...
describe('admin redaction invariant', () => {
  it('the admin service never selects a credential column', () => {
    const service = code('../services/admin-two-factor.service.ts');
    for (const column of ['codeHash', 'passwordHash', 'refreshTokenHash', 'tokenHash', 'code_hash']) {
      ...
      assert.equal(
        ...
        false,
        `admin-two-factor.service.ts references ${column} in executable code`,
      );
    }
  });

  it('the admin service selects columns explicitly, never a whole row', () => {
    // `db.select().from(users)` expands to EVERY column Drizzle knows about, including
    // password_hash and refresh_token_hash. A bare select() is how a hash would reach a response
    // without anyone naming it — which is exactly what the check above cannot catch.
    assert.equal(
      /\.select\(\s*\)/.test(code('../services/admin-two-factor.service.ts')),
      false,
      'admin-two-factor.service.ts contains a bare .select() — it would pull every column',
    );
  });

  it('the admin controller spreads no raw row', () => {
    const controller = code('../controllers/admin-two-factor.controller.ts');
...
describe('event vocabulary invariant', () => {
  ...
  it('the admin action names are permitted by their own CHECK constraint', () => {
    ...
    // change — a rejected action name rolls the whole action back rather than gapping the trail.
