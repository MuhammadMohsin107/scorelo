...
import { getAltTextConfig, previewAltText, saveAltTextConfig } from '../services/altText.service.js';
import { optionalStoreId, requireUserId } from '../lib/requestContext.js';

/**
 * Image alt-text template configuration.
 *
 * All three handlers are store-scoped through the services, which resolve the caller's own store —
...
export async function postAltTextPreview(req: Request, res: Response) {
  res.json({
    data: await previewAltText(requireUserId(req), req.body.contentType, req.body.config, optionalStoreId(req)),
  });
}
